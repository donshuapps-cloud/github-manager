...
**Donshu**, veo dos problemas:

1. **El script no detecta GitHub CLI** aunque probablemente está instalado
2. **El prompt muestra "License" extraño** - es un artefacto de la línea anterior

---

### 🔧 Verificar GitHub CLI

Primero, ejecuta estos comandos para diagnosticar:
...
### 📝 Corregir el script de autenticación

El problema es que `auth.sh` tiene una función `check_gh_cli` pero no se está usando correctamente en `repo-init.sh`. Corrijo ambos:
...
**Donshu**, primero verifica GitHub CLI:
...
# 3. Autenticar GitHub CLI (usará tu SSH)
gh auth login
# Selecciona:
#   > GitHub.com
#   > SSH
#   > Use existing SSH key (Enter)

# 4. Verificar autenticación
gh auth status

# 5. Ejecutar el script nuevamente
...
**Resultado esperado:**
```
📁 INICIALIZAR REPOSITORIO EN PROYECTO EXISTENTE

📂 Ruta del proyecto (actual: ...): /home/soporte/Development/recetas_system
[WARN] ℹ️  El repositorio ya está inicializado
¿Deseas continuar y configurar GitHub? (s/n): s

📝 Nombre del repositorio: recetas_system
📄 Descripción (opcional): Sistema modular...
🔒 ¿Repositorio privado? (s/n): s

[INFO] Verificando autenticación de GitHub CLI...
✅ GitHub CLI autenticado correctamente
[INFO] Creando repositorio con GitHub CLI...
✅ Repositorio creado y sincronizado con GitHub CLI
✅ Repositorio inicializado exitosamente
```

¿Ahora funciona, **Donshu**?